...
const setExAsync = promisify(redisClient.setex)
...
async function main(ctx: Context, next: Function) {
    ...
    try {
        const result = await setExAsync(
            deploymentStore.project.name,
            getSecondsUntil(deploymentStore.project.timeToDeploy) + 10, // add a buffer of 10s so that we overcommit to value. This is better than undercomitting in cases where network overhead causes delays.
